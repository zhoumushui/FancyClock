package com.zms.fancyclock.view;
...
import com.zms.fancyclock.R;
import com.zms.fancyclock.event.EmptyEvent;
import com.zms.fancyclock.event.TimeUpdateEvent;
import com.zms.fancyclock.service.LyClockService;
...
public class GoogleClock extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (attrs != null) {
			TypedArray typedArray = context.obtainStyledAttributes(attrs,
					R.styleable.LyGoogleClock, defStyleAttr, 0);
			try {
				mClockWidth = typedArray.getDimension(
						R.styleable.LyGoogleClock_lyClockWidth, 0.0f);
				mHourWidth = typedArray.getDimension(
						R.styleable.LyGoogleClock_lyHourWidth, 0.0f);
				mMinWidth = typedArray.getDimension(
						R.styleable.LyGoogleClock_lyMinWidth, 0.0f);
				mSecWidth = typedArray.getDimension(
						R.styleable.LyGoogleClock_lySecWidth, 0.0f);
				mDividerHorizontal = typedArray.getDimension(
						R.styleable.LyGoogleClock_lyDividerHorizontal, 0.0f);
				mDividerVertical = typedArray.getDimension(
						R.styleable.LyGoogleClock_lyDividerVertical, 0.0f);
				mTheme = typedArray.getResourceId(
						R.styleable.LyGoogleClock_lyTheme, -1);
			} finally {
				typedArray.recycle();
			}
		}
	...
	private void initView() {
		...
		if (mDividerHorizontal != 0.0f) {
			ViewGroup.LayoutParams params = mViewDividerHorizontal0
					.getLayoutParams();
		...
		if (mDividerVertical != 0.0f) {
			ViewGroup.LayoutParams params = mViewDividerVertical
					.getLayoutParams();
	...
	private void updateClock(TimeUpdateEvent event) {
		...
		if (mHourTenDigits != event.getHourTenDigits()) {
			updateImageView(mIvHourTenDigits,
					mResList.get(event.getHourTenDigits()));
			mHourTenDigits = event.getHourTenDigits();
		}
		if (mHourSingleDigits != event.getHourSingleDigits()) {
			updateImageView(mIvHourSingleDigits,
					mResList.get(event.getHourSingleDigits()));
			mHourSingleDigits = event.getHourSingleDigits();
		}
		if (mMinTenDigits != event.getMinTenDigits()) {
			updateImageView(mIvMinTenDigits,
					mResList.get(event.getMinTenDigits()));
			mMinTenDigits = event.getMinTenDigits();
		}
		if (mMinSingleDigits != event.getMinSingleDigits()) {
			updateImageView(mIvMinSingleDigits,
					mResList.get(event.getMinSingleDigits()));
			mMinSingleDigits = event.getMinSingleDigits();
		}
		if (mSecTenDigits != event.getSecTenDigits()) {
			updateImageView(mIvSecTenDigits,
					mResList.get(event.getSecTenDigits()));
			mSecTenDigits = event.getSecTenDigits();
		}
		if (mSecSingleDigits != event.getSecSingleDigits()) {
			updateImageView(mIvSecSingleDigits,
					mResList.get(event.getSecSingleDigits()));
	 ...
	 * 
	 * @param target
	 *            which image view to anim
	 * @param animTo
	 *            anim to which vector
	 */
	private void updateImageView(ImageView target, int animTo) {
		...
		if (drawable instanceof AnimatedVectorDrawable) {
			...
			if (mHourTenDigits != -1 && mHourSingleDigits != -1
					&& mMinTenDigits != -1 && mMinSingleDigits != -1
					&& mSecTenDigits != -1) {
			...
			// ,
			// todo 11:11, 22:22(there are some same numbers) , only the first
			// can work correctly
